...
shared_examples 'a celestial body fetch test', type: :request do |id:, type:, expected_status:, expected_response: {}|
  before(:all) do
  ...
  before(:each) do
    create(:user) do |user|
      ...
      get "/api/v1/celestial/#{type}/#{id}", headers: headers, as: :json
    end
  end
