...
import { MoonData, PlanetData, RingedPlanetData, StarData } from "@/views/create/util/types";
...
export default defineComponent({
  ...
  props: {
    ...
    data: Object as PropType<StarData | MoonData | PlanetData | RingedPlanetData>,
  },
  setup(props, { emit }) {
    ...
    watch(assetsLoaded, () => {
      ...
      const z = size.length();
      const camPos: Vector3 = new Vector3(z, z, z * 10);
    ...
    const bodyProps = computed(() => {
      ...
      if (props.type === 'asteroid belt') keys = keys.concat("radius", "luminosity");
      const object = objectWithoutKeys(data.value, ...keys);
      const scaled = scaleParams<typeof props.data>(object, props.type);
    ...
    const beltAttributes = computed(() => {
      if (props.type === 'asteroid belt') return attributes.value;
    ...
    const scaler = props.type === 'moon' ? 1 : 1e3;
    const intensity = SOLAR_LUMINOSITY * LIGHTING_SCALE * scaler;
    const lightDist = DISTANCE_SCALE * SUN_EARTH_RADIUS_RATIO;
    const lightPos = props.type === 'asteroid belt' ? { x: 0, y: 0, z: 0 } : { x: -lightDist, y: 0, z: lightDist };
